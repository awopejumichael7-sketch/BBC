...
async function renderLive() {
  ...
  sessionUnsub = onSnapshot(sessionRef, (snap) => {
    ...
    if (!studentPc) {
      ...
      document.getElementById("join-live").onclick = () => joinLive(sessionRef);
    }
  });
}
...
async function joinLive() {
  ...
  wrap.innerHTML = `
    ...
    <div class="studio-controls"><button class="btn-danger" id="leave-live">Leave Live Class</button></div>
    <p style="color:var(--muted);font-size:.85rem;margin-top:8px;">This is a live broadcast and is not being recorded.</p>`;
  protectElement(wrap);
  document.getElementById("leave-live").onclick = leaveLive;

  studentPc = new RTCPeerConnection(ICE_CONFIG);
  studentPc.addTransceiver("video", { direction: "recvonly" });
  studentPc.addTransceiver("audio", { direction: "recvonly" });
  ...
  await setDoc(viewerDocRef, { offer: { type: offer.type, sdp: offer.sdp }, joinedAt: serverTimestamp() });
